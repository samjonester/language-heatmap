...
describe RepoLister do
	...
	let(:next_response_data) { double('next_response_data', items: [{ full_name: 'xdd/cuddly-waffle'  }]) }
	let(:next_response_value) { double('next_response_value', data: next_response_data) }
	...
	it "Should list repositories by language" do
		expect(client).to receive(:search_repos).with("language:#{language}", per_page: 10000).and_return(search_response)
